...
export default function CartProduct({ product }) {
    ...
    return (
        <div className="flex justify-between space-x-4 hover:shadow-lg hover:border-opacity-50
                        border-opacity-0 rounded-md p-4 bg-white">
            ...
            <div className="flex items-center">
                ...
                <button 
                        ...
                        disabled={product.quantity <= 1}  className="p-1 rounded-md hover:bg-rose-100 hover:text-rose-500">
                                <MinusSmallIcon className="w-6 h-6 flex-shrink-0" />
                            </button>
                            <p className="font-semibold text-xl">{product.quantity}</p>
                            <button
                            onClick={() => setItemQuantity(product.id, product.quantity + 1)}
                            className="p-1 rounded-md hover:bg-rose-100 hover:bg-green-100">
